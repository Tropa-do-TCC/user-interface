...
from reconstruction import setup_skull
...
class AppWindow(QtWidgets.QMainWindow, QtWidgets.QApplication):
    def __init__(self, app):
        ...
        self.brain = setup_skull(self.renderer, self.nii_file_path)
    ...
    def add_vtk_widget(self):
        ...
        base_title = f"Crânio: {base_brain_file}"

        wrapper_group_box = QtWidgets.QGroupBox()
        wrapper_layout = QtWidgets.QVBoxLayout()

        vtk_group_box = QtWidgets.QGroupBox(base_title)
    ...
    def create_file_selector(self, label, window_title, name_filter):
        file_import_button = QtWidgets.QPushButton(label)
        file_import_button.clicked.connect(lambda _: self.open_file(window_title, name_filter))
        
        return file_import_button


    def open_file(self, window_title, name_filter):
    ...
    def add_skull_settings_widget(self):
        ...
        skull_file_selector = self.create_file_selector("Importar arquivo NIFTI", 'Selecionar arquivo NIFTI', 'Arquivos nii.gz (*.nii.gz)')
        skull_group_layout.addWidget(skull_file_selector, 1, 0, 1, 3)

        skull_group_layout.addWidget(self.create_separator(), 2, 0, 1, 3)

        skull_opacity_slider = self.create_slider(0, 1, self.brain.property.GetOpacity() * 100, self.set_skull_opacity)
    ...
    def add_landmarks_settings_widget(self):
        ...
        detect_landmarks_button = QtWidgets.QPushButton("Detectar pontos fiduciais")
        detect_landmarks_button.clicked.connect(lambda _: None)
        ...
        landmarks_file_selector = self.create_file_selector("Importar pontos fiduciais", 'Selecionar JSON com pontos fiduciais', 'Arquivos JSON (*.json)')
        ...
        landmarks_group_layout.addWidget(landmarks_visible_checkbox, 4, 0, 1, 3)
    ...
    def set_skull_opacity(self, opacity_value):
        self.brain.property.SetOpacity(opacity_value / 100)
        self.render_window.Render()
    ...
    def set_axial_view(self):
        ...
        distance = math.sqrt((position[0] - focal_point[0]) ** 2 + (position[1] - focal_point[1]) ** 2 + (position[2] - focal_point[2]) ** 2)
    ...
    def set_sagittal_view(self):
        ...
        position = self.renderer.GetActiveCamera().GetPosition()
        distance = math.sqrt((position[0] - focal_point[0]) ** 2 + (position[1] - focal_point[1]) ** 2 + (position[2] - focal_point[2]) ** 2)
        self.renderer.GetActiveCamera().SetPosition(focal_point[2] + distance, focal_point[0], focal_point[1])
