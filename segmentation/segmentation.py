...
def segment_slice(dcm_file_path, input_dir, current_bio, current_dim, current_q, current_gamma, output_dir):
    ...
    transformed_pixel_array = dicomHandler.image_transformation(
        pixel_array, current_gamma)
    ...
    hist, bin_edges, best_thresholds, img_thres = wspMultithreshold.wspMultithreshold(
        hu_image, current_bio, current_dim, current_q)
    ...
    dicomHandler.save_dicom(dicom_image, pixel_array,
                            f'{output_dir}/{dcm_file_path}')
...
def segmentate(image_folder, bio_algorithm, dimension, q, gama):
    ...
    output_dir = input_dir + \
        f"-{current_date}-{bio_algorithm}-d{dimension}-q{q}"
    os.mkdir(output_dir)

    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(segment_slice, all_slices, repeat(input_dir), repeat(
            bio_algorithm), repeat(dimension), repeat(q), repeat(gama), repeat(output_dir))
