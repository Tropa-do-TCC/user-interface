...
def wspArtificialBeeColony(n, dim, maxGeneration, hist, lb, ub, objFunc, q):
    ...
    while t < maxGeneration:
        ...
        for i in range(n):
            ...
            new_bee.Position = bees[i].Position + phi * \
                (np.array(bees[i].Position) - np.array(bees[k].Position))

            # Make sure each individual is legal.
            new_bee.Position = FeasibleFunction(new_bee.Position, dim, lb, ub)

            # Evaluation
            new_bee.Cost = CostFunction(
                new_bee.Position.astype(int), hist, lb, ub, objFunc, q)
        ...
        for i in range(n):
            if C[i] >= L:
                ...
                bees[i].Position = bee
                bees[i].Cost = CostFunction(
                    bees[i].Position.astype(int), hist, lb, ub, objFunc, q)
                C[i] = 0
